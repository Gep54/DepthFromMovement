...
def run_descriptor_landmark_pipeline(
    ...
    *,
    ...
    """
    Multi-baseline pairing (same as sequence export), landmark fusion in metric world frame.

    Writes ``run_dir/descriptor_map/landmarks_topdown.png``,
    ``run_dir/descriptor_map/sparse_depth_ref_cam.png`` (frame 0 projection), and optional ``iter/*.png``.
    """
    root = Path(run_dir)
...
def _write_final_plots(dm_root: Path, ds: Dataset, desc_map: DescriptorLandmarkMap) -> None:
    ...
    uv, z = project_world_to_camera_uv_z(K, xyz, ds.world_T_camera[0])
    mask = np.isfinite(uv[:, 0]) & (z > 1e-9)
    panel = estimated_depth_visualization(und0, uv[mask], z[mask])
    cv2.imwrite(str(dm_root / "sparse_depth_ref_cam.png"), panel)
...
def _write_snapshot(
    ...
    mask = np.isfinite(uv[:, 0]) & (z > 1e-9)
    panel = estimated_depth_visualization(und0, uv[mask], z[mask])
    cv2.imwrite(str(iter_root / f"sparse_depth_ref_cam_{stem}.png"), panel)

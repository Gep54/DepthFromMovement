"""Load map_3d parameters from configuration.env with CLI precedence."""
...
def apply_config_to_argv(argv: list[str] | None = None) -> list[str]:
    ...
    merged_known = {k: v for k, v in {**file_known, **cli_known}.items() if v.strip() != ""}
...
def _warn(message: str) -> None:
    print(f"[map_3d_ros2] warning: {message}", file=sys.stderr)
